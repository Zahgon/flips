...
public class FlipDescriptionController {
    ...
    private List<FlipDescription> getAllFlipDescription(String featureName) {
        return getAllMethodsCached().stream().filter(getFeatureFilter(featureName)).map(this::buildFlipDescription).sorted(Comparator.comparing(FlipDescription::getMethodName)).collect(toList());
    }
    ...
    private FlipDescription buildFlipDescription(Method method) {
        return new FlipDescription(method.getName(), method.getDeclaringClass().getName(), flipAnnotationsStore.isFeatureEnabled(method));
    }
}
